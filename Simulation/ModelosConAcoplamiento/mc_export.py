...
def build_latex(ctx: CouplingContext) -> str:
    ...
    section_intros = {
        ...
        "Generalizacion EQT I: lagrangiano e invariantes": (
            "Se declara una suma finita arbitraria de invariantes de las torres "
            "$\\alpha_n$ y $\\beta_m$. Las reglas se componen por linealidad antes "
            "de elegir la funcion metrica."
        ),
        "Generalizacion EQT II: motor sobre el ansatz": (
            "El motor tensorial comun construye los momentos totales y las "
            "ecuaciones coordenadas manteniendo $f(r)$ sin resolver."
        ),
        "Generalizacion EQT III: rama y verificacion": (
            "Las ecuaciones se reducen a una primera integral, se selecciona la "
            "rama analitica y se sustituye en todos los momentos e invariantes."
        ),
    }
...
def export_results(ctx: CouplingContext, compile_pdf: bool = True) -> dict[str, object]:
    ...
    tex_path = output_dir / "derivacion_modelos_con_acoplamiento_eqt_general.tex"
    pdf_path = output_dir / "derivacion_modelos_con_acoplamiento_eqt_general.pdf"
