...
def bloque_ecuacion(lhs, rhs):
    """Ajusta expresiones largas al ancho de la página."""
    ecuacion = rf"{lhs} = {rhs}"
    if len(ecuacion) > 220:
        return (
            "\\[\n"
...
RESUMEN_PRINCIPAL = [
    ...
    (
        "Objetos geométricos calculados",
        [
    ...
    (
        "Palatini y sustitución de la conexión",
        [
    ...
    (
        "Primera y segunda integración por partes",
        [
            (r"I_{\mathrm{inicio}}", "ibp_start"),
            (r"\nabla_j B_1^{\,j}", "ibp1_divergence"),
            (r"R_1", "ibp1_residual_positive"),
            (r"\nabla_j B_2^{\,j}", "ibp2_divergence"),
            (r"I_{\mathrm{bulk}}", "ibp2_bulk_Hup"),
...
def export_results(
    ...
    if carpeta_salida is None:
        carpeta_salida = Path.cwd() / "salidas_fR"
    else:
...
\begin{center}
{\Large\bfseries Resumen de la derivación simbólica para teorías \(f(R)\)}\\[0.5em]
...
"""
    ...
    snippet_path = carpeta_salida / "ecuaciones_principales_fR.tex"
    tex_path = carpeta_salida / "resumen_derivacion_fR.tex"
    pdf_path = carpeta_salida / "resumen_derivacion_fR.pdf"
    ...
    display(FileLink(
        str(tex_path),
        result_html_prefix="Documento LaTeX completo: "
    ))
    display(FileLink(
        str(snippet_path),
        result_html_prefix="Fragmento LaTeX reutilizable: "
    ))

    if compilado_pdf:
        display(FileLink(
            str(pdf_path),
            result_html_prefix="PDF compilado: "
        ))
    elif compilar_pdf:
        display(Markdown(
